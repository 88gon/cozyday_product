...
function populatePicker(el, start, end, selectedIndex) {
  ...
  for (let i = start; i <= end; i++) {
    html += `<div class="picker-item" data-value="${i}">${i}${el.id.includes('year') ? '년' : el.id.includes('month') ? '월' : el.id.includes('day') ? '일' : el.id.includes('hour') ? '시' : '분'}</div>`;
  }
...
function getPickerValue(id) {
  ...
  const scrollPos = el.scrollTop;
  const index = Math.round(scrollPos / 40);
  const activeItem = el.querySelectorAll('.picker-item')[index + 1];

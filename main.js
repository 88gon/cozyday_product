...
function calculateSaju() {
  ...
  setTimeout(() => {
      ...
      renderKeywordResult(resultData, dayPillar, saju.getDayGan().toString());
  }, 1500);
}
...
function renderKeywordResult(data, dayPillar, dayGan) {
  ...
  // MBTI 소울메이트 로직 (인연 섹션에 참고용으로 사용 가능)
  ...
  let html = `
    <div class="tiger-comment" style="background: #fdf5e6; padding: 15px; border-radius: 10px; margin-bottom: 20px; border-left: 5px solid #8d6e63; font-size: 0.95em; line-height: 1.6;">
      ...
      "${data.tigerComment || '오, 꽤 괜찮은 기운을 타고났는데? ...물론 내가 옆에서 지켜줘서 더 잘 풀리는 거겠지만.'}"
    </div>
    <div class="core-summary">
      <h3>${data.title}</h3>
      <div class="tags">${data.tags.join(' ')}</div>
      <p>${data.coreSummary}</p>
    </div>
  `;

  // 키워드별 섹션 배치 (오행, 성격, 재물, 직업, 인연, 성공 전략)
  const categories = [
